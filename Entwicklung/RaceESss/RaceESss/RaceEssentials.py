...
configPath = "apps/python/RaceEssentials/config.ini"

config = configparser.ConfigParser()
config.read(configPath)

if config.has_section("RaceEssentials") != True:
	config.add_section("RaceEssentials")
	updateConfig = True
	
def getOrSetDefaultBoolean(config, key, default):
	global updateConfig
	try:
		return config.getboolean("RaceEssentials", key)
	except:
		config.set("RaceEssentials", key, str(default))
		updateConfig = True
		return default
...
def getOrSetDefaultFloat(config, key, default):
	global updateConfig
	try:
		return config.getfloat("RaceEssentials", key)
	except:
		config.set("RaceEssentials", key, str(default))
		updateConfig = True
		return default
...
personalBestDir = "apps/python/RaceEssentials/personal_best/"
compoundsPath = "apps/python/RaceEssentials/compounds/"
configDir = "apps/python/RaceEssentials/config/"

template = "apps/python/RaceEssentials/images/template.png"
abson = "apps/python/RaceEssentials/images/abs.png"	
absoff = "apps/python/RaceEssentials/images/absoff.png"
tcon = "apps/python/RaceEssentials/images/tc.png"
tcoff = "apps/python/RaceEssentials/images/tcoff.png"
drs_available = "apps/python/RaceEssentials/images/drs_available.png"
drs_enabled = "apps/python/RaceEssentials/images/drs_enabled.png"
drs_off = "apps/python/RaceEssentials/images/drs_off.png"
...
def acMain(ac_version):
	...
	if maxPowerRpmLights:
		try:
			...
			ac.console("RaceEssentials: UTF ui_car.json failed to load")
			try:
				...
				ac.console("RaceEssentials: ANSI ui_car.json failed to load")
	...
	#App window
	appWindow = ac.newApp("RaceEssentials")
	...
	ac.setPosition(PossibleNewLaptimLable, 382 * scale, 80 * scale)
	...
	ac.setPosition(personalBestLapLabel, 233 * scale, 75 * scale)
	...
	ac.setFontSize(tyreLabelWearFL, 13 * scale)
	...
	ac.setFontSize(tyreLabelWearFR, 13 * scale)
	...
	ac.setFontSize(tyreLabelWearRL, 13 * scale)
	...
	ac.setFontSize(tyreLabelWearRR, 13 * scale)
	...
	ac.setFontSize(tyreLabelTempFL, 13 * scale)
	...
	ac.setFontSize(tyreLabelTempFR, 13 * scale)
	...
	ac.setFontSize(tyreLabelTempRL, 13 * scale)
	...
	ac.setFontSize(tyreLabelTempRR, 13 * scale)
	...
	ac.setFontSize(tyreLabelPresFL, 13 * scale)
	...
	ac.setFontSize(tyreLabelPresFR, 13 * scale)
	...
	ac.setFontSize(tyreLabelPresRL, 13 * scale)
	...
	ac.setPosition(sessionTimeLabel, 496 * scale, 25 * scale)
	ac.setFontSize(sessionTimeLabel, 13 * scale)
	ac.setCustomFont(sessionTimeLabel, "Consolas", 0, 1)
	
	#System clock
	systemClockLabel = ac.addLabel(appWindow, "Clk: --:--")
	ac.setPosition(systemClockLabel, 489 * scale, 11 * scale)
	...
	ac.setPosition(temperatureLabel, 127 * scale, 83 * scale)
	...
	ac.setPosition(trackGripLabel, 112 * scale, 97 * scale)
	...
	ac.setPosition(tyreCompoundLabel, 112 * scale, 111 * scale)
	...
	ac.setPosition(deltaButton, 396 * scale, 16 * scale)
			...
			if timerDisplay > 1:
				...
				if previousTyreCompoundValue != tyreCompoundValue:
					...
					if compounds.has_section(carValue + "_" + tyreCompoundShort.lower()):
						...
						#ac.setText(TestLable, "{:2.2f} : {:2.2f} : {:2.2f} : {:2.2f}".format(idealPressureFront, idealPressureRear, minimumOptimalTemperature, maximumOptimalTemperature))
...
def onFormRender(deltaT):
	...
	#Delta gain bar
	ac.glColor4f(1, 1, 1, 0.3)
	ac.glQuad(365 * scale, 70 * scale, 96 * scale, 6 * scale)
	if deltaButtonValue:
		if ttpb > 0:
			ac.glColor4f(1, 0.18, 0.18, 1)
			deltaOffset = min(ttpb * 1000, 48)
			ac.glQuad((413 - deltaOffset) * scale, 70 * scale, deltaOffset * scale, 6 * scale)
		if ttpb < 0:
			ac.glColor4f(0.18, 1, 0.18, 1)
			deltaOffset = min(abs(ttpb * 1000), 48)
			ac.glQuad(413 * scale, 70 * scale, deltaOffset * scale, 6 * scale)
	else:
		if ttb > 0:
			ac.glColor4f(1, 0.18, 0.18, 1)
			deltaOffset = min(ttb * 1000, 48)
			ac.glQuad((413 - deltaOffset) * scale, 70 * scale, deltaOffset * scale, 6 * scale)
		if ttb < 0:
			ac.glColor4f(0.18, 1, 0.18, 1)
			deltaOffset = min(abs(ttb * 1000), 48)
			ac.glQuad(413 * scale, 70 * scale, deltaOffset * scale, 6 * scale)
